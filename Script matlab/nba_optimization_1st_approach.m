...
clear all
n = 30;
nomeFile = "C:\Users\sandra\PC\Desktop\politecnico\Tesi_Omento\nba_schedule23-24.xlsx";

% Imposta la data di inizio
dataInizio = datenum('Tue, Oct 24, 2023', 'ddd, mmm dd, yyyy');
...
%   Scegli casualmente due giorni diversi che non siano in 'no_game_days'
    schedule_iniz = best_schedule;
    schedule = schedule_iniz;
    while true
        day1 = randi(num_giorni);
        day2 = randi(num_giorni);
        
        % Verifica che day1 e day2 siano diversi tra loro
        if day2 ~= day1
            % Verifica che day1 e day2 non siano in no_game_days
            if ~ismember(day1, no_game_days) && ~ismember(day2, no_game_days)
                break; % Esci dal ciclo se soddisfa tutte le condizioni
            end
        end
    end
...
%   Scambio tra partite 
    if ~isequal(temp, [0, 0]) && isequal(schedule{team1, day2}, [0,0]) && isequal(schedule{team2, day2}, [0,0])
        schedule{team1, day1} = schedule{team1, day2};
        schedule{team1, day2} = temp;
        schedule{team2, day1} = schedule{team2, day2};
        schedule{team2, day2} = temp;
    else 
        continue;
    end
%   Controlli ammissibilità
    consecutive_count = zeros(n, 1);
    for i = 1:num_squadre
        % Controllo back to back
        max_back_to_back = true;
        count_back_to_back = 0;
        for j = 1:num_giorni - 1
            if ~isequal(schedule{i, j}, [0, 0]) && ~isequal(schedule{i, j + 1}, [0, 0])
                count_back_to_back = count_back_to_back + 1;
            end
        end
        consecutive_count(i) = count_back_to_back;
        back2back = max(consecutive_count);
        if back2back > 18
            max_back_to_back = false;
            break;
        end
        % Controllo 3 partite di fila
        tre_di_fila = true;
        for j = 1:num_giorni - 2
            count_tre_di_fila = 0;
            for k = j:j+2
                if ~isequal(schedule{i, k}, [0, 0])
                    count_tre_di_fila = count_tre_di_fila + 1;
                end
            end
            if count_tre_di_fila == 3
                tre_di_fila = false;
            end
            if ~tre_di_fila
                break;
        ...
        % Controllo massimo 5 partite in 8 giorni
        five_in_eight = true;
        for j = 1:num_giorni - 7
            count_five_in_eight = 0;
            for k = j:j+7
                if ~isequal(schedule{i, k}, [0, 0])
                    count_five_in_eight = count_five_in_eight + 1;
                end
            end
            if count_five_in_eight > 5
                five_in_eight = false;
                break;
            end
        end
    ...
    % Se il nuovo travel è migliore, accetta lo scambio altimrenti ripristina la configurazione precedente
    if (sum(new_travel) < sum(best_travel))
        best_travel = new_travel;
        best_schedule = schedule;
        scambi_effettuati = scambi_effettuati + 1;
    else
        best_schedule = schedule_iniz;
    end
end
